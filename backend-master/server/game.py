...
from server.game_modifiers import FlipGrid, Symbols, VirusStormField, TraceStormField, Alien
...
from utils.special_commands import DummyTraceCommand, DummyVirusCommand, SpecialCommand
...
class Slot:
    def __init__(self, client, ready=False, host=False):
        ...
        self.defeating_trace = False
        self.defeating_virus = False
    ...
    async def reset_trace(self, after=2):
        await asyncio.sleep(after)
        self.defeating_trace = False

    async def reset_virus(self, after=2):
        await asyncio.sleep(after)
        self.defeating_virus = False
...
class Game:
    ...
    async def join_client(self, client):
        ...
        if len(self.slots) >= self.max_players:
            await Sio().emit("game_join_fail", {
                "message": "Session full — connection refused"
            }, room=client.sid)
            return
    ...
    async def next_level(self):
        ...
        if self.level > 0:
            ...
            self.difficulty["asteroid_chance"] = 0.15   # DummyTraceCommand chance
            self.difficulty["black_hole_chance"] = 0.15  # DummyVirusCommand chance
            ...
            if random.random() < self.difficulty["game_modifier_chance"]:
                ...
                cls = random.choice(
                    list(filter(
                        ...
                        [Symbols, FlipGrid, TraceStormField, VirusStormField, Alien]
                    ))
                )
    ...
    async def intro_done_all(self):
        ...
        await Sio().emit("command", {
            "text": "Boot up and stand by for instructions.",
            "time": warmup_time
        }, room=self.sio_room)
    ...
    async def complete_instruction(self, instruction_completed, increase_health=True):
        ...
        if self.health >= 100:
            ...
            await Sio().emit("next_level", {
                ...
                "text": self.game_modifier.DESCRIPTION if self.game_modifier is not None else "No anomalies detected. Gibson is quiet."
            }, room=self.sio_room)
        else:
    ...
    async def defeat_special(self, client, virus=False):
        # Playing/player checks
        if not self.playing:
            raise RuntimeError("Game not in progress!")
        slot = self.get_slot(client)
        ...
        # Record the defeat action
        if virus:
            slot.defeating_virus = True
        else:
            slot.defeating_trace = True

        # Check if everyone is defeating
        all_defeated = True
        for s in self.slots:
            if (not s.defeating_virus and virus) or (not s.defeating_trace and not virus):
                all_defeated = False
                break

        # Everyone has defeated the trace/virus!
        if all_defeated:
            logging.debug("All defeated!")

            # Check if there's a special command (we may have more than one)
            instructions_completed = []
            for instruction in self.instructions:
                if (type(instruction.target_command) is DummyVirusCommand and virus) \
                        or (type(instruction.target_command) is DummyTraceCommand and not virus):
        ...
        asyncio.Task(slot.reset_virus() if virus else slot.reset_trace())
