...
async def join_game(sid, data, client):
    if data["game_id"].lower() not in LobbyManager():
        ...
        await sio.emit("game_join_fail", {
            "message": "Session not found — check your uplink"
        })
    else:
...
@server.link_client
@server.client_in_game_in_progress
async def defeat_trace(sid, data, client):
    logging.debug("Got a trace alert!")
    await client.game.defeat_special(client, virus=False)
...
@server.link_client
@server.client_in_game_in_progress
async def defeat_virus(sid, data, client):
    logging.debug("Got a virus attack!")
    await client.game.defeat_special(client, virus=True)
